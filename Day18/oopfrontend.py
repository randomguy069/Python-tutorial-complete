from tkinter import *
from oopbackend import Database #importing from the backend script
...
def view_command():
    ...
    for row in database.view(): #accessing each tuple from the list returned from backend.view
        list1.insert(END,row) #inserting from the end

def search_command():
    list1.delete(0,END)
    for row in database.search(title_text.get(),author_text.get(), isbn_text.get(), year_text.get()):
        list1.insert(END,row)

def add_command():
    database.insert(title_text.get(),author_text.get(), isbn_text.get(), year_text.get())
    list1.delete(0,END)
...
def delete_command():
    ...
    #view_command()
def update_command():
    database.update(selected_tuple[0],title_text.get(),author_text.get(), isbn_text.get(), year_text.get())
    #view_command()
